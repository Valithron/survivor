class_name RosterSpriteVisual
extends Node2D

## Shared presentation adapter for Ryan and Cooper's 8-direction player sheets.
## Gameplay owns the animation name; this node only resolves an atlas cell and
## exposes the established hurt/death/basic reaction seam.
const CELL_SIZE := 64
...
func play_basic_shot(muzzle_side: int = 0) -> void:
	if _action == "death":
		return
	_forced_basic_frame = 1 if muzzle_side < 0 else 0
...
func _apply_frame() -> void:
	var active_action := "hurt" if _reaction_remaining > 0.0 else _action
