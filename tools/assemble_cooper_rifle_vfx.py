"""Normalize Cooper's two-frame automatic-rifle tracer into a small atlas."""
...
def _normalize(source: Image.Image) -> Image.Image:
	...
	rgb = frame.convert("RGB").quantize(colors=20, method=Image.Quantize.MEDIANCUT).convert("RGBA")
	rgb.putalpha(alpha)
	return rgb


def main() -> None:
	source_path = ROOT / "art/source/cooper_rifle_tracer_generated_source_v1.png"
	output_path = ROOT / "art/vfx/cooper/cooper_rifle_tracer_sheet_v1.png"
	source = Image.open(source_path).convert("RGBA")
	if source.width % FRAME_COUNT != 0 or source.height <= 0:
		raise ValueError("Cooper tracer source must contain two equal horizontal cells.")
	...
	for index in range(FRAME_COUNT):
		...
		if frame.getbbox() is None:
			raise ValueError("Cooper tracer source contains an empty frame.")
