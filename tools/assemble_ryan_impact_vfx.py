"""Assemble Ryan's three-stage radial impact atlas from its reviewed source."""
...
def _normalize(source: Image.Image) -> Image.Image:
	...
	rgb = frame.convert("RGB").quantize(colors=28, method=Image.Quantize.MEDIANCUT).convert("RGBA")
	rgb.putalpha(alpha)
	return rgb


def main() -> None:
	source_path = ROOT / "art/source/ryan_radial_impact_generated_source_v1.png"
	output_path = ROOT / "art/vfx/ryan/ryan_radial_impact_sheet_v1.png"
	source = Image.open(source_path).convert("RGBA")
	if source.width % FRAME_COUNT != 0 or source.height <= 0:
		raise ValueError("Ryan impact source must contain three equal horizontal cells.")
	...
	for index in range(FRAME_COUNT):
		...
		if frame.getbbox() is None:
			raise ValueError("Ryan impact source contains an empty frame.")
