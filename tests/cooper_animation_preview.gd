...
## Development-only visual review room for Cooper's authored 64 px animation
## sheet. It is intentionally not part of the production scene flow.
const DIRECTIONS := ["n", "ne", "e", "se", "s", "sw", "w", "nw"]
const ACTIONS := ["idle", "run", "basic", "hurt", "death"]
const VISUAL_SCENE: PackedScene = preload("res://art/characters/cooper/cooper_visual.tscn")

@onready var hero: RosterSpriteVisual = $Hero
@onready var gameplay_scale: RosterSpriteVisual = $GameplayScale
...
var _action_index := 0
var _direction_strip: Array[RosterSpriteVisual] = []


func _ready() -> void:
	for index in range(DIRECTIONS.size()):
		var visual := VISUAL_SCENE.instantiate() as RosterSpriteVisual
...
func _apply_preview_state() -> void:
	...
	title_label.text = "COOPER ANIMATION REVIEW  //  %s  //  %s" % [action.to_upper(), direction.to_upper()]
